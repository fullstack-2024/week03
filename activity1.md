...
In this activity, you'll follow the steps below to build a responsive webpage as in the screenshot:
...
- Create a directory named `week3-lab01` and extract the [images](./img1.zip) into that directory. 
...
```
> **Step 6** <br>
Next, give each `img` a `src` attribute according to its order in the document. The first `img` should have a `src` of `./img/1.jpg`. The rest should be the same, except replace the `1` with the number the `img` is in the document.
